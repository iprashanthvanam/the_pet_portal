...
function fetchCartInfo() {
    fetch("/api/cart/")
        .then(res => res.json())
        .then(data => {
            const item = data.items.find(it => it.type === 'food' && it.id === itemId);
...
function fetchFoodDetails() {
    fetch(`/api/foods/${itemId}/`)
        ...
        .then(data => {
            ...
            const weight = data.weight_kg ? (data.weight_kg + " kg") : "10 kg";
            const fullTitle = `${brand} Premium ${data.food_type} Pet Food for ${data.name}, ${weight} Bag`;
            document.getElementById("food-display-title").innerText = fullTitle;
            ...
            specsTable.innerHTML = `
                <tr>
                    <td class="label-cell">Flavour Base</td>
                    <td class="val-cell">${data.name || "Original Flavour"}</td>
                </tr>
                <tr>
                    <td class="label-cell">Food Type Style</td>
                    <td class="val-cell">${data.food_type || "Kibble"}</td>
                </tr>
                <tr>
                    <td class="label-cell">Life Stage Profile</td>
                    <td class="val-cell">All Life Stages (Puppy/Adult/Senior)</td>
                </tr>
                <tr>
                    <td class="label-cell">Net Mass Volume</td>
                    <td class="val-cell">${data.weight_kg ? data.weight_kg + ' kg Bag' : '10 kg Bag'}</td>
                </tr>
                <tr>
                    <td class="label-cell">Dietary Focus</td>
                    <td class="val-cell">High-Protein, Grain-Free Profile</td>
                </tr>
                <tr>
                    <td class="label-cell">Mfg Date</td>
                    <td class="val-cell">${new Date(data.mfg_date).toLocaleDateString("en-IN", {day: 'numeric', month: 'short', year: 'numeric'})}</td>
                </tr>
                <tr>
                    <td class="label-cell">Expire Date</td>
                    <td class="val-cell" style="color:#22c55e;">${new Date(data.expire_date).toLocaleDateString("en-IN", {day: 'numeric', month: 'short', year: 'numeric'})}</td>
                </tr>
            `;
            ...
            bullets.innerHTML = `
                <li>Contains high-quality ingredients rich in protein and vitamins.</li>
                <li>Grain-free formulation to support sensitive pet digestive tracts.</li>
                <li>Rich in essential Omega-3 and Omega-6 fatty acids for a glossy pedigree coat.</li>
                <li>Manufactured by certified nutritional partners and vetted veterinary practitioners.</li>
            `;
            ...
            if (data.reviews && data.reviews.length > 0) {
                data.reviews.forEach(review => {
                    ...
                    if (isSuper || userRole === 'master_admin' || userRole === 'product_seller' || userRole === 'food_seller') {
                        replyFormHTML = `
                ...
                feed.innerHTML = `<p style="color: var(--text-muted); font-size: 13px;">No reviews left yet. Be the first to review this food item!</p>`;
            }
        })
        .catch(err => console.error("Error loading food details:", err));
}
...
function updateCartItem(quantity) {
    ...
    .then(data => {
        if (data.success) {
            const item = data.cart.find(it => it.type === 'food' && it.id === itemId);
            userCartQty = item ? item.quantity : 0;
            renderActionBox();
