    ...
    dashboardData.bookings.forEach(booking => {
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td>Boarding care for ${booking.pet_name} starting ${new Date(booking.start_datetime).toLocaleDateString()} with User ID ${booking.user}</td>
                <td><span class="badge-status" style="background: rgba(245,158,11,0.2); color:#f59e0b; padding:4px 10px; border-radius:20px; font-weight:600; font-size:11px;">${booking.status}</span></td>
            </tr>
        `;
    });
...
function renderBookings() {
    ...
    dashboardData.bookings.forEach(booking => {
        ...
        let actionButtons = `
            <button class="btn-moderate" onclick="openCareBookingModal(${booking.id})" style="padding: 4px 8px; font-size: 12px; margin-right:5px;"><i class="fa-solid fa-edit"></i> Edit</button>
        `;
        if (booking.status === "PENDING") {
            ...
            actionButtons += `
                <button class="btn-moderate" onclick="updateStatus(${booking.id}, 'ACTIVE', this)" style="padding: 4px 8px; font-size: 12px; margin-right:5px; background:var(--primary);">Check In</button>
            `;
        } else if (booking.status === "ACTIVE") {
            actionButtons += `
                <button class="btn-moderate" onclick="updateStatus(${booking.id}, 'COMPLETED', this)" style="padding: 4px 8px; font-size: 12px; margin-right:5px; background:#3b82f6; color:white;">Check Out</button>
            `;
        }
        ...
        tbody.innerHTML += `
            <tr>
                <td>
                    ...
                    <span style="font-size: 12px; color: var(--text-muted)">${booking.pet_species} | Age: ${booking.pet_age}</span>
                </td>
                <td>${booking.username}</td>
                <td>
                    <span style="font-size: 13px">From: ${new Date(booking.start_datetime).toLocaleDateString()}</span><br>
                    <span style="font-size: 13px">To: ${new Date(booking.end_datetime).toLocaleDateString()}</span>
                </td>
                <td>
                    <strong>${booking.total_days} Days</strong><br>
                    <span style="font-size: 13px; color: #10b981">₹${booking.total_price}</span>
                </td>
                <td>
    ...
    if (!hasVisibleBookings) {
        tbody.innerHTML = `<tr><td colspan="6" style="text-align: center; color: var(--text-muted)">No bookings scheduled.</td></tr>`;
    }
...
window.updateStatus = function(bookingId, status, btnElement = null) {
    let endpoint = status === 'CANCELLED' 
        ? `/api/bookings/care/${bookingId}/cancel/`
        : `/api/bookings/care/${bookingId}/status/`;
    ...
    .catch(err => {
        ...
        console.error("Error updating care status:", err);
    });
};

window.openCareBookingModal = function(id = null) {
    ...
    if (id) {
        fetch(`/api/bookings/care/${id}/`)
            ...
            .then(data => {
                ...
                document.getElementById("care-start-date").value = formatDate(data.start_datetime);
                document.getElementById("care-end-date").value = formatDate(data.end_datetime);
                document.getElementById("care-total-days").value = data.total_days;
                document.getElementById("care-total-price").value = data.total_price;
...
window.closeCareBookingModal = function() {
    document.getElementById("care-booking-modal").style.display = "none";
    document.getElementById("care-booking-form").reset();
};

window.saveCareBooking = function(e) {
    e.preventDefault();
    ...
    const payload = {
        ...
        start_datetime: document.getElementById("care-start-date").value + "T00:00:00Z",
        end_datetime: document.getElementById("care-end-date").value + "T00:00:00Z",
        total_days: document.getElementById("care-total-days").value,
        total_price: document.getElementById("care-total-price").value,
    ...
    filtered.forEach(review => {
        ...
        let replyFormHTML = `
            <form onsubmit="submitReviewReply(event, ${review.id})" style="margin-top: 10px; display: flex; gap: 8px; width: 100%;">
                <input type="text" placeholder="Write reply as Pet Care..." required style="flex: 1; background: var(--bg-body); border: 1px solid var(--border-color); color: var(--text-main); padding: 6px 10px; border-radius: 6px; font-size: 12px;">
