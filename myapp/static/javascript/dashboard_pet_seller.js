...
window.switchPanel = function(panelId) {
    ...
    // Set active sidebar item based on panel
...
function loadDashboard() {
    fetch("/api/dashboard/pet-seller/")
        .then(res => res.json())
        .then(data => {
            dashboardData = data;
    ...
    dashboardData.orders.forEach(o => {
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td><span class="badge-status" style="background: rgba(99,102,241,0.2); color:#6366f1; padding: 4px 10px; border-radius: 20px; font-size: 11px; font-weight:600;">${o.status}</span></td>
            </tr>
        `;
    });
...
function renderPets() {
    ...
    dashboardData.listings.forEach(pet => {
        const name = (pet.name || "").toLowerCase();
        const species = (pet.species || "").toLowerCase();
        const microchip = (pet.microchip_id || "-").toLowerCase();

        if (searchVal && !name.includes(searchVal) && !species.includes(searchVal) && !microchip.includes(searchVal)) {
            return;
        }
            ...
            <tr>
                <td>
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <img src="${pet.image || 'https://via.placeholder.com/50'}" alt="${pet.name}" style="width: 40px; height: 40px; border-radius: 6px; object-fit: cover;">
                        <strong>${pet.name}</strong>
                    </div>
                </td>
                <td>${pet.species}</td>
                <td>${pet.microchip_id || "-"}</td>
                <td>₹${pet.price}</td>
                <td>
                    <button class="btn-moderate" onclick="openModal(${pet.id})" style="padding: 4px 8px; font-size: 12px; margin-right: 5px;"><i class="fa-solid fa-edit"></i> Edit</button>
                    <button class="btn-moderate btn-danger" onclick="deletePet(${pet.id})" style="padding: 4px 8px; font-size: 12px;"><i class="fa-solid fa-trash"></i> Delete</button>
                </td>
            </tr>
        `;
    });
    ...
    if (!hasVisiblePets) {
        tbody.innerHTML = `<tr><td colspan="5" style="text-align: center; color: var(--text-muted)">No pets found.</td></tr>`;
    }
}
    ...
    if (id) {
        fetch(`/api/admin/pets/${id}/`)
            ...
            .then(item => {
                ...
                document.getElementById("form-species").value = item.species;
...
window.saveItem = function(e) {
    ...
    formData.append("species", document.getElementById("form-species").value);
    ...
    filtered.forEach(review => {
        ...
        let replyFormHTML = `
            <form onsubmit="submitReviewReply(event, ${review.id})" style="margin-top: 10px; display: flex; gap: 8px; width: 100%;">
                <input type="text" placeholder="Write reply as Pet Seller..." required style="flex: 1; background: var(--bg-body); border: 1px solid var(--border-color); color: var(--text-main); padding: 6px 10px; border-radius: 6px; font-size: 12px;">
