...
function loadDashboard() {
    fetch("/api/dashboard/product-seller/")
        .then(res => res.json())
        .then(data => {
            dashboardData = data;
...
function renderFoods() {
    ...
    dashboardData.listings.forEach(food => {
        const name = (food.name || "").toLowerCase();
        const type = (food.food_type || "").toLowerCase();

        if (searchVal && !name.includes(searchVal) && !type.includes(searchVal)) {
            return;
        }
            ...
            <tr>
                <td>
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <img src="${food.image || 'https://via.placeholder.com/50'}" alt="${food.name}" style="width: 40px; height: 40px; border-radius: 6px; object-fit: cover;">
                ...
                <td>₹${food.price}</td>
                <td>
                    <button class="btn-moderate" onclick="openModal(${food.id})" style="padding: 4px 8px; font-size: 12px; margin-right: 5px;"><i class="fa-solid fa-edit"></i> Edit</button>
                    <button class="btn-moderate btn-danger" onclick="deleteFood(${food.id})" style="padding: 4px 8px; font-size: 12px;"><i class="fa-solid fa-trash"></i> Delete</button>
                </td>
            </tr>
        `;
    });
    ...
    if (!hasVisibleFoods) {
        tbody.innerHTML = `<tr><td colspan="4" style="text-align: center; color: var(--text-muted)">No food products found.</td></tr>`;
    }
}
...
window.openModal = function(id = null) {
    ...
    document.getElementById("modal-title").innerText = id ? "Edit Food" : "Add Food";
    ...
    if (id) {
        fetch(`/api/admin/foods/${id}/`)
            ...
            .then(item => {
                ...
                document.getElementById("form-type").value = item.food_type;
                ...
                document.getElementById("form-weight").value = item.weight_kg || "";
                document.getElementById("form-expire").value = item.expire_date || "";
...
window.saveItem = function(e) {
    ...
    formData.append("food_type", document.getElementById("form-type").value);
    ...
    formData.append("weight_kg", document.getElementById("form-weight").value);
    formData.append("mfg_date", document.getElementById("form-mfg").value);
    formData.append("expire_date", document.getElementById("form-expire").value);
    ...
    .catch(err => console.error("Error saving food item:", err));
};
    ...
    filtered.forEach(review => {
        ...
        if (review.image) {
            reviewImageHTML = `
                <div style="margin-top: 8px;">
                    <img src="${review.image}" alt="Review photo" style="max-width: 60px; max-height: 60px; border-radius: 4px; object-fit: cover; cursor: pointer; border: 1px solid var(--border-color);" onclick="window.open('${review.image}', '_blank')">
                </div>
            `;
        }
        ...
        let replyFormHTML = `
            <form onsubmit="submitReviewReply(event, ${review.id})" style="margin-top: 10px; display: flex; gap: 8px; width: 100%;">
                <input type="text" placeholder="Write reply as Product Seller..." required style="flex: 1; background: var(--bg-body); border: 1px solid var(--border-color); color: var(--text-main); padding: 6px 10px; border-radius: 6px; font-size: 12px;">
