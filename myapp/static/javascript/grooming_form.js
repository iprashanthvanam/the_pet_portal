    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        form.addEventListener('submit', function (e) {
            ...
            const formData = {
                ...
                pet_size: form.querySelector('[name="pet_size"]').value,
                package_type: form.querySelector('[name="package_type"]').value,
                visit_type: form.querySelector('[name="visit_type"]').value,
                preferred_groomer: form.querySelector('[name="preferred_groomer"]')?.value || '',
                appointment_datetime: form.querySelector('[name="appointment_datetime"]').value,
            };
            ...
            .then(({ ok, data }) => {
                ...
                if (ok) {
                    showMessage('success', `Grooming booked! Total: ₹${data.total_price || 0}. Redirecting...`);
                    form.reset();
                    setTimeout(() => {
                        window.location.href = '/grooming/history/';
                    }, 1500);
                } else {
        ...
        function loadReviews() {
            fetch("/api/reviews/?grooming_id=all")
                .then(res => res.json())
                .then(reviews => {
                        ...
                        reviews.forEach(review => {
                            ...
                            if (rMedia.length > 0) {
                                if (rMedia.length === 1) {
                                    ...
                                    const carouselId = `grooming-review-carousel-${review.id}`;
                            ...
                            if (isSuper || userRole === 'master_admin' || userRole === 'pet_grooming') {
                                replyFormHTML = `
        ...
        window.submitReview = function(e) {
            ...
            formData.append("service_type", "GROOMING");
